...
void TableroGL::DrawCell(int i, int j) {
	...
	bool isSelected = (selectedPiece && i == selected_x && j == selected_y);
	bool isPossibleMove = false;

	for (const auto& pos : posiblesMovimientos) {
	...
	if (isSelected) {
		...
		glColor3f(0.0, 8.0, 0.0);  // Color blanco para la casilla seleccionada
		glBegin(GL_QUADS);
		glVertex3f(glx - width / 2, gly - width / 2, 0.001);
		glVertex3f(glx + width / 2, gly - width / 2, 0.001);
		glVertex3f(glx + width / 2, gly + width / 2, 0.001);
		glVertex3f(glx - width / 2, gly + width / 2, 0.001);
		glEnd();
	...
	if (isPossibleMove) {
		glDisable(GL_LIGHTING);
		glColor3f(0.0, 1.0, 0.0);  // Color verde para los posibles movimientos
		glBegin(GL_TRIANGLE_FAN);
		glVertex3f(glx, gly, 0.01);
		for (int k = 0; k <= 20; k++) {
			glVertex3f(
				glx + (width / 4) * cos(k * 2 * M_PI / 20),
				gly + (width / 4) * sin(k * 2 * M_PI / 20),
				0.01
			);
		}
		glEnd();
...
void TableroGL::MouseButton(int x, int y, int button, bool down, bool sKey, bool ctrlKey) {
	...
	if (down && button == MOUSE_LEFT_BUTTON) {
		if (selectedPiece) {
			if (!m_tablero->movePiece(selected_x, selected_y, xcell_sel, ycell_sel)) {
			...
			// Verificar si la pieza seleccionada es blanca
			if (m_tablero->getTab()[xcell_sel][ycell_sel].getColor() == "BLANCO") {
				selected_x = xcell_sel;
				selected_y = ycell_sel;

				play("sonidos/cogerpieza.mp3");

				selectedPiece = true;
				posiblesMovimientos = m_tablero->posicionesPosibles(selected_x, selected_y);
			}
			else {
				cout << "Solo puede mover piezas blancas. Intente nuevamente." << endl;
			}
		}
	}
...
void TableroGL::moveRandomBlackPiece() {
	...
	while (!piezasNegras.empty()) {
		...
		while (attempts < 1000) {
			int dx = rand() % 17 - 8; // -1, 0, 1
			int dy = rand() % 17 - 8; // -1, 0, 1
			if (dx != 0 || dy != 0) {
				int to_x = from_x + dx;
				int to_y = from_y + dy;
		...
		if (movimientoHecho) {
			...
			cout <<"cant Negras = "<< piezasNegras.size() << endl;
			/*if (piezasNegras.size()==0) {
				play("sonidos/jaquemate.mp3");
