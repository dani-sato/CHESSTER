...
void Reina::mover_izq(int casillas, int color) { //Izquierda de la pantalla, no del punto de vista del peon
	if (getColor() == 1 && tablero.esPosOcupadaPorBlanca(getPosx() - casillas, getPosy()) == false) { //Esto funciona bien para controlar los limites del tablero 
		Pieza::mover_izq(casillas);

	//	ReinaBx = ReinaBx - casillas;
	}
	else if (getColor() == 0 && tablero.esPosOcupadaPorNegra(getPosx() - casillas, getPosy()) == false) { //Esto funciona bien para controlar los limites del tablero 
		Pieza::mover_izq(casillas);
...
void Reina::mover_arriba(int casillas, int color) {
	if (getColor() == 1 && tablero.esPosOcupadaPorBlanca(getPosx(), getPosy() + casillas) == false) { //Esto funciona bien para controlar los limites del tablero 
		Pieza::mover_arriba(casillas);

		//ReinaBy = ReinaBy + casillas;
	}
	else if (getColor() == 0 && tablero.esPosOcupadaPorNegra(getPosx(), getPosy() + casillas) == false) { //Esto funciona bien para controlar los limites del tablero 
		Pieza::mover_arriba(casillas);
...
void Reina::mover_arr_dcha(int casillas, int color) {
	if (getColor() == 1 && tablero.esPosOcupadaPorBlanca(getPosx() + casillas, getPosy() + casillas) == false) {
	...
	else if (getColor() == 0  && tablero.esPosOcupadaPorNegra(getPosx() + casillas, getPosy() + casillas) == false) {
		Pieza::mover_arr_dcha(casillas);
